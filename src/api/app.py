...
async def create_job(
    ...
    if written > MAX_UPLOAD_BYTES:
        ...
        raise HTTPException(
            status_code=413,
            detail=f"{name} is {written / 1e6:.1f} MB, over the "
            f"{MAX_UPLOAD_BYTES / 1e6:.0f} MB transcription limit.",
        )

    job = store.create(destination.name)
    background.add_task(process, job, destination, settings, store)
    log.info("queued job %s for %s", job.id, destination.name)
    return job.as_dict()
